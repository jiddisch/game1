...
export default class Level1 extends BaseLevelScene {
  ...
  createLevel() {
    ...
    this.physics.add.overlap(this.player, this.obstacles, () => this.handlePlayerHit("נפלת לחור! 😢"), null, this);
  ...
  decideHoleTile() {
    ...
    for (const c of candidates) {
      if (this.isFloorTile(c.x, c.y)) return c;
    }
    return null;
  }

  openHoleAt(x, y) {
    if (this.currentHole && (this.currentHole.x !== x || this.currentHole.y !== y)) {
